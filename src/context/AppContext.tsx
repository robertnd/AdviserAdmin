...
const Context = ({ children }: { children: React.ReactNode }) => {
  ...
  const [requestHeaders, setRequestHeaders] = useState<Params[]>([
    { id: 1, key: "", value: "" },
  ]);
  ...
  const [responseDetails, setResponseDetails] = useState<AxiosResponse | null>(
    null
  );
  ...
  const handleAdd = (paramsType: string) => {
    if (paramsType === "queryParams") {
      setQueryParams([
        ...queryParams,
        { id: queryParams.length + 1, key: "", value: "" },
      ]);
    } else if (paramsType === "requestHeaders") {
      setRequestHeaders([
        ...requestHeaders,
        { id: requestHeaders.length + 1, key: "", value: "" },
      ]);
    }
  };
