...
export class AccountService {
  ...
  constructor(private router: Router, private http: HttpClient) {
    this.userSubject = new BehaviorSubject(JSON.parse(localStorage.getItem('omadmin')!))
  ...
  login(username: string, password: string) {
    ...
    .pipe(map(user => {
      if (user.authenticated) {
        localStorage.setItem('omadmin', JSON.stringify(user))
  ...
  logout() {
    ...
    localStorage.removeItem('omadmin')
